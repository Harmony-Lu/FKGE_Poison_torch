...
class TensorTrainDataset(TensorDataset):
    def __init__(self, triples, nentity, negative_sample_size):
        self.len = triples.shape[0]
        ...
        for h, r, t in triples:
            self.hr2t[(h.item(), r.item())].add(t.int().item())
    ...
    def __getitem__(self, idx):
        ...
        while negative_sample_size < self.negative_sample_size:
            ...
            import random
            negative_sample = random.sample(range(0, self.nentity), self.negative_sample_size*2)
            # 从self.hr2t[(head, relation)] 返回 negative_sample 是否在其中
            true_tail_list = self.hr2t[(head.item(), relation.item())]
            negative_sample = list(set(negative_sample) - set(true_tail_list))
            negative_sample_list = negative_sample_list + negative_sample
            negative_sample_size += len(negative_sample)
        negative_sample = torch.tensor(negative_sample_list[:self.negative_sample_size])
