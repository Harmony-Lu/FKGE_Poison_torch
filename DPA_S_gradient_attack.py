...
class Server(object):
    ...
    def server_inner_train(self):
        ...
        # total_loss = 0.0
        # batch_num = 0
        # self.poison = Parameter(torch.Tensor(1,3)) # 预计添加10个有毒三元组
        # # 1表示使用训练集中对应三元组
        # # 0表示不使用训练集中对应三元组
        # self.poison.data.fill_(1) 
    ...
    def server_poison_data_generate(self, ent_meta_grad):
        ...
        for ent in row_grad_sum_idx:
            ...
            if ent in head_list:
                ...
                # print("Not found this entity as head entity in train dataset!")
                continue
        ...
        # 3.The poisoned triplet are saved in file
    ...
    def train_poison_model(self,clients):
        ...
        if self.train_dataloader == None: # 向shadow model投毒
            ...
            # attack_model = Metattack(self.args, self.poisoned_triples, self.rel_embed, self.poisoned_ent_embed, self.victim_client_ent_embed)
            # poisoned_triples = attack_model()
            # print(type(poisoned_triples))
            # print(len(poisoned_triples))
    ...
    def train(self):
        ...
        for num_round in range(self.args.max_round):
            ...
            if bad_count >= self.args.early_stop_patience:
                ...
                break
            if num_round%15==0 and num_round != 0: # 每隔15轮，用 测试集数据 检验当前embedding的质量
